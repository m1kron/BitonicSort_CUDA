...
// Thread mask encodes inforamtion wether given thread should take min or max
// from values.
...
// It is assumed that threadRegisterBuffer contains every 32's element of given
// bucket.
...
__device__ int PerformStep_32ElementsWide_sync(const int val) {
  const int otherVal =
      __shfl_xor_sync(tools::ALL_THREADS_MASK, val, STEP_OFFSET);
...
#pragma unroll
    for (int storageIdx = 0; storageIdx < REGISTER_BUFFER_SIZE; ++storageIdx) {
      ...
      if (compareToIdx < storageIdx) continue;

      if (helpers::NeedsToBeSwapped<int, PRODUCE_INCREASING>(
              threadRegisterBuffer[storageIdx],
              threadRegisterBuffer[compareToIdx]))
        helpers::Swap(threadRegisterBuffer[storageIdx],
                      threadRegisterBuffer[compareToIdx]);
    }
  }
...
#pragma unroll
  for (int i = 0; i < REGISTER_BUFFER_SIZE; ++i) {
    threadRegisterBuffer[i] =
        warp::internal::PerformStage_WarpWide_sync<PRODUCE_INCREASING>(
            threadRegisterBuffer[i]);
  }
}
