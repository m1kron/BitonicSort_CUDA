...
// NOTE: all following functions assume that will operate on warp that calling
// thread belongs to. NOTE2: It is assumed that all thread's in given warp will
// be active when calling any of the following functions.

namespace warp {
// Performs bitonic sort over sequence of BUCKET_SIZE items that is stored at
// warpSharedMemPtr. Result os sorted incresing/decreasing sequence of
// BUCKET_SIZE. warpSharedMemPtr has to point to shared memory.
...
// Performs bitonic sort's stage over sequence of BUCKET_SIZE items that is
// stored at warpSharedMemPtr. Result is sorted incresing/decreasing sequence of
// BUCKET_SIZE. warpSharedMemPtr has to point to shared memory.
template <int BUCKET_SIZE, bool PRODUCE_INCREASING>
...
__device__ void PerformBitonicSort_BucketSizeWide_sync(int* warpSharedMemPtr) {
  static_assert(BUCKET_SIZE == 256,
                "Current implementation supports only 256 bucket size!");
...
#pragma unroll
  for (int i = 0; i < itemsPerThread; ++i)
    storage[i] = warpSharedMemPtr[tools::GetThreadIdxWithinWarp() +
                                  i * tools::WARP_SIZE];

// Stages 1 - 5.
#pragma unroll
  for (int i = 0; i < itemsPerThread; i += 2) {
    storage[i] = warp::internal::PerformBitonicSort_32ElementsWide_sync<true>(
        storage[i]);
    storage[i + 1] =
        warp::internal::PerformBitonicSort_32ElementsWide_sync<false>(
            storage[i + 1]);
  }
...
#pragma unroll
  for (int i = 0; i < itemsPerThread; i += 4) {
    ...
    warp::internal::PerformStageForGivenBucketSize_sync<64, false>(storage + i +
                                                                   2);
  }
  ...
  // Stage 8.
  warp::internal::PerformStageForGivenBucketSize_sync<256, PRODUCE_INCREASING>(
      storage);

// Store date to sharedMem.
#pragma unroll
  for (int i = 0; i < itemsPerThread; ++i)
    warpSharedMemPtr[tools::GetThreadIdxWithinWarp() + i * tools::WARP_SIZE] =
        storage[i];
...
#pragma unroll
  ...
  // Sort in registers.
  warp::internal::PerformStageForGivenBucketSize_sync<BUCKET_SIZE,
                                                      PRODUCE_INCREASING>(
      storage);
...
#pragma unroll
  for (int i = 0; i < ITEMS_PER_THREAD; ++i) {
    ...
    const int offset2 =
        bucketOffset + i * tools::WARP_SIZE + tools::GetThreadIdxWithinWarp();
    const int val1 = sharedMem[offset1];
    const int val2 = sharedMem[offset2];

    const bool swapNeeded =
        helpers::NeedsToBeSwapped<int, PRODUCE_INCREASING>(val1, val2);
    if (swapNeeded) {
