...
__global__ void SortBitionicInSharedMemoryKernel(int4* data, uint32_t dataSize) 
{
  __shared__ int4 sharedMem[KERNEL_PARAMS::SHARED_SIZE_INT4];

  for(uint32_t thisBlockOffset = blockIdx.x*KERNEL_PARAMS::SHARED_SIZE_INT4; thisBlockOffset < dataSize; thisBlockOffset += gridDim.x*KERNEL_PARAMS::SHARED_SIZE_INT4)
  {
    const int thisBlockBucketIdx = thisBlockOffset/KERNEL_PARAMS::SHARED_SIZE_INT4;
    const bool increasing = (thisBlockBucketIdx&1)==0;

    block::SortBitionicInSharedMemory<KERNEL_PARAMS::COMPARE_SWAP_WARP_BUCKET_SIZE, KERNEL_PARAMS::WARP_BUCKET_SIZE>(data + thisBlockOffset, sharedMem, KERNEL_PARAMS::SHARED_SIZE_INT4, increasing);
  }
}

////////////////////////////////////////////////////////////////////////////////////
template<typename KERNEL_PARAMS>
__global__ void PerformBitonicLastStagesInSharedMemoryKernel(int4* data, uint32_t dataSize, int originStage) 
{
  __shared__ int4 sharedMem[KERNEL_PARAMS::SHARED_SIZE_INT4];

  const int COMPARE_SWAP_BLOCK_BUCKET_SIZE = KERNEL_PARAMS::SHARED_SIZE_INT4/2;
  ...
  for(uint32_t thisBlockOffset = blockIdx.x*KERNEL_PARAMS::SHARED_SIZE_INT4; thisBlockOffset < dataSize; thisBlockOffset += gridDim.x*KERNEL_PARAMS::SHARED_SIZE_INT4)
    block::PerformBitonicStageInSharedMemory<KERNEL_PARAMS::COMPARE_SWAP_WARP_BUCKET_SIZE, KERNEL_PARAMS::WARP_BUCKET_SIZE>(data + thisBlockOffset, sharedMem, KERNEL_PARAMS::SHARED_SIZE_INT4, increasing);
}

////////////////////////////////////////////////////////////////////////////////////
template<typename KERNEL_PARAMS>
__global__ void PerformBitonicStageKernel(int4* data, uint32_t dataSize, int step, int stage)
{
  const int COMPARE_SWAP_BLOCK_BUCKET_SIZE = KERNEL_PARAMS::SHARED_SIZE_INT4/2;
  ...
  int thisBlockGlobalMemOffset = thisBlockBucketIdx*step + (blockIdx.x%blocksPerBucket)*COMPARE_SWAP_BLOCK_BUCKET_SIZE;
  const int currentBlocksStride = blockDim.x/blocksPerBucket*step;
  for( ;thisBlockGlobalMemOffset < dataSize; thisBlockGlobalMemOffset += currentBlocksStride )
  {
...
////////////////////////////////////////////////////////////////////////////////////
void BitonicSort(int32_t* in_out_gpuDataPtr, uint32_t dataSize)
{
  ...
  SortBitionicInSharedMemoryKernel<PARAMS><<<numOfBlocks,THREADS_PER_BLOCK>>>(in_out_gpuDataPtr_int4,dataSize_int4);

  for( int stage = PARAMS::SHARED_SIZE_INT4*2; stage <= dataSize_int4; stage *= 2 )
  {
    for( int step = stage; step > PARAMS::SHARED_SIZE_INT4; step /= 2)
      PerformBitonicStageKernel<PARAMS><<<numOfBlocks,PARAMS::THREADS_PER_BLOCK>>>(in_out_gpuDataPtr_int4,dataSize_int4,step,stage);
    PerformBitonicLastStagesInSharedMemoryKernel<PARAMS><<<numOfBlocks,PARAMS::THREADS_PER_BLOCK>>>(in_out_gpuDataPtr_int4,dataSize_int4,stage);
  }
}
